...
export const Assdess =()=>{
    ...
    let asp=()=>{
        let y=[...arr].sort((a,b)=>{console.log(a) 
            console.log(b)
            return a.Price-b.Price ;})
        setArr(y)
    }
    let dsp=()=>{
        let y=[...arr].sort((a,b)=>{return b.Price-a.Price})
        setArr(y)
    }
    let asn=()=>{
        let y=[...arr].sort((p,q)=>{
            if(p.Name.toUpperCase()<q.Name.toUpperCase()){
                return -1;
            }
        })
        setArr(y)
    ...
    let dsn=()=>{
        let y=[...arr].sort((p,q)=>{ return q.Name.toUpperCase()<p.Name.toUpperCase()?-1:1;
        })
        setArr(y)
    }
    return(
        <div>
            ...
            <div>
                <button onClick={()=>{asn()}}>Assending Name</button>
                <button onClick={()=>{dsn()}}>Dessending Name</button>
                <button onClick={()=>{asp()}}>Assending Price</button>
                <button onClick={()=>{dsp()}}>Dessending Price</button>
            </div>
            <div style={{display:"flex",flexWrap:"wrap",width:"85%",margin:"auto"}}>
                {arr.map((e,i)=>{
                    return(
                        <div key={i} style={{width:"20%",flexWrap:"wrap",padding:"10px",boxShadow:"0 0 10px #000"}}>
